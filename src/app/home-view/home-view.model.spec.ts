...
describe('HomeViewModel', () => {
  ...
  let countriesDataService: CountriesDataService;
  beforeEach(() => {
    countriesDataService = {
      countries: of(getCountries()),
    } as any;
    model = new HomeViewModel(countriesDataService);
  });

  describe('countries', () => {
    it('Returns all countries when no filters applied', () => {
      return model.countries
        .pipe(
          first(),
          tap((countries: Country[]) => {
            expect(countries.length).toBe(getCountries().length);
            expect(countries[0].name).toBe(getCountries()[0].name)
          })
        )
        .toPromise();
    });

    it('Applies search filter if set', () => {
      model.updateCountrySearch('Afghanistan');
      return model.countries
        .pipe(
          first(),
          tap((countries: Country[]) => {
    ...
    it('Supports partial match on search filter', () => {
      model.updateCountrySearch('g');
      return model.countries
        .pipe(
          first(),
          tap((countries: Country[]) => {
    ...
    it('Supports case-insensitive search filter', () => {
      model.updateCountrySearch('AFGHANISTAN');
      return model.countries
        .pipe(
          first(),
    ...
    it('Applies region filter if set', () => {
      model.updateCountryRegion('Asia');
      return model.countries
        .pipe(
          first(),
          tap((countries: Country[]) => {
            ...
            expect(countries[0].name).toBe('Afghanistan');
            expect(countries[1].name).toBe('Armenia');
          })
        )
        .toPromise();
    });

    it('Applies both search and region filters if set', () => {
      model.updateCountrySearch('Armenia');
      model.updateCountryRegion('Asia');
      return model.countries
        .pipe(
          first(),
          tap((countries: Country[]) => {
            expect(countries.length).toBe(1);
            expect(countries[0].name).toBe('Armenia');
          })
        )
        .toPromise();
    });
